...
function InboxPage() {
  ...
  const items = window.DATA.inbox;
  const filtered = filter === 'all' ? items : items.filter(i =>
    filter === 'today' ? !['Yest','Mon','Sun','Sat'].includes(i.when) :
    filter === 'reading' ? i.kind === 'pa' || i.kind === 'do' || i.kind === 'po' || i.kind === 'ar' :
    items
  );
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <h1 className="page-title">Inbox<span className="ital"> — 47 items pending triage</span></h1>
        ...
        <div>
          <div className="inbox-toolbar">
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <span className={'filter-pill' + (filter === 'all' ? ' on' : '')} onClick={() => setFilter('all')}>All<span className="num">47</span></span>
          ...
          <div className="inbox-list">
            {filtered.map(it => (
              ...
              >
                ...
                <div className="src-icon" title={it.source}>{it.source[0]}</div>
                <div className="body">
                  {it.body}
                  <span className="src-meta">{it.meta}</span>
                </div>
                <div className="suggest">
                  <span className="muted-2">→</span>
                  <span>route to <b style={{ color: 'var(--ink)', fontWeight: 500 }}>{it.suggest.space}</b></span>
                </div>
        ...
        <aside className="inbox-side">
          ...
          <div className="side-card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                <span className="tag"><span className="swatch"></span>To read</span>
                <span style={{ fontSize: 11.5, color: 'var(--ink-4)' }}>12 pending</span>
              </div>
            </div>
          </div>
